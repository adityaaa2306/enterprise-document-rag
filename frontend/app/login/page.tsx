...
function LoginForm() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      try {
        const { upgradeGuestAfterLogin, getGuestSessionId } = await import(
          "@/lib/guest-session"
        )
        if (getGuestSessionId()) {
          await upgradeGuestAfterLogin()
        }
      } catch {
        // Upgrade is best-effort; login still succeeds
      }
      ...
      <div className="p-8">
        <div className="flex flex-col items-center mb-8">
          ...
          <p className="text-muted-foreground text-center text-sm">
            Sign in to Sustainability Manager
          </p>
        </div>
...
export default function LoginPage() {
  return (
    <div className="min-h-screen bg-background flex items-center justify-center p-6">
      <Suspense
        fallback={
          <Card className="w-full max-w-md border-border bg-card shadow-xl p-8 text-center text-muted-foreground text-sm">
            Loading…
          </Card>
        }
      >
