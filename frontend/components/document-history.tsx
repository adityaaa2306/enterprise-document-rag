...
/** Module cache so Dashboard remounts do not refetch /documents every time. */
let docsCache: { at: number; docs: Document[] } | null = null
let docsInflight: Promise<Document[]> | null = null
const DOCS_TTL_MS = 60_000

async function loadDocuments(force = false): Promise<Document[]> {
  if (!force && docsCache && Date.now() - docsCache.at < DOCS_TTL_MS) {
  ...
  docsInflight = (async () => {
    try {
      ...
      if (!res.ok) return docsCache?.docs || []
      const data = await res.json()
      const docs = Array.isArray(data) ? (data as Document[]) : []
...
export function DocumentHistory() {
  const [documents, setDocuments] = useState<Document[]>(() => docsCache?.docs || [])
  ...
  if (loading && documents.length === 0) {
    return <div className="text-white/50">Loading history...</div>
  }
