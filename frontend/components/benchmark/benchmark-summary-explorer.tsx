...
function SummaryResponseCard({ run }: { run: QuestionRun }) {
  const summaryText = (run.summary || run.answer || "").trim()
  return (
    <div className="rounded-lg border border-border/45 bg-card/50 p-3.5 flex flex-col min-h-[240px]">
      ...
      <div className="grid grid-cols-2 gap-1.5 text-[11px] mb-3">
        ...
        <Metric
          label="Length"
          value={
            run.summary_length == null && run.summary_chars == null
              ? "—"
              : `${fmtNum(run.summary_length ?? run.summary_chars, 0)} chars`
          }
        />
        ...
        <Metric
          label="Grounded"
          value={
            (run.quality?.groundedness ?? run.groundedness) == null
              ? "—"
              : fmtNum(run.quality?.groundedness ?? run.groundedness, 1)
          }
        />
      </div>

      <div className="flex-1 rounded-md bg-black/25 border border-border/30 px-3 py-2.5 overflow-auto max-h-56">
        {run.error ? (
...
export function BenchmarkSummaryExplorer({
  questions,
}: {
  ...
  return (
    <Card className="p-6 bg-gradient-to-br from-card to-card/50 border-border/50">
      <div className="mb-5">
        <h3 className="text-lg font-semibold">Summary explorer</h3>
        <p className="text-xs text-muted-foreground mt-1">
          Expand to compare every participant side-by-side — same frozen document and
          prompt, different summarization strategies. Metrics and summaries are loaded
          from stored artifacts only.
        </p>
      </div>
      ...
      <div className="space-y-2">
        {questions.map((q, idx) => {
          ...
          const reference = q.reference_summary || q.reference_answer
          return (
            <Collapsible
              key={id}
              open={open}
